...
//            cn = obtenerConexionJTDS();
            ...
            while (rs.next()) {
                impuesto = impuesto + Utilitarios.parseDouble_Null(rs.getString("valor"));
            }
        } finally {
            cerrar(cn, cl, rs);
        }
...
//            cn = obtenerConexionJTDS();
            ...
            while (rs.next()) {
                if (lcPrecio_Aplicado.equalsIgnoreCase("PE")) {
                    l = Utilitarios.parseDouble_Null(rs.getString("precio_stk"));
                }
                if (lcPrecio_Aplicado.equalsIgnoreCase("P1")) {
                    l = Utilitarios.parseDouble_Null(rs.getString("precio1"));
                }
                if (lcPrecio_Aplicado.equalsIgnoreCase("P2")) {
                    l = Utilitarios.parseDouble_Null(rs.getString("precio2"));
                }
                if (lcPrecio_Aplicado.equalsIgnoreCase("P3")) {
                    l = Utilitarios.parseDouble_Null(rs.getString("precio3"));
                }
                if (lcPrecio_Aplicado.equalsIgnoreCase("P4")) {
                    l = Utilitarios.parseDouble_Null(rs.getString("precio4"));
                }
                if (lcPrecio_Aplicado.equalsIgnoreCase("P5")) {
                    l = Utilitarios.parseDouble_Null(rs.getString("precio5"));
                }
            }
        } finally {
            cerrar(cn, cl, rs);
...
//            cn = obtenerConexionJTDS();
    ...
    public List<Object[]> buscar_tg30campaniaveh() throws Exception {//no se usa
        ...
        try {
            ...
            while (rs.next()) {
                ...
                s[0] = Utilitarios.parseInteger_Null(rs.getString("idtg30campaniaveh"));
    ...
    public Double buscar_tcambio(String fecha) throws Exception {//no se ua
        ...
        try {
            ...
            rs = pr.executeQuery();
            while (rs.next()) {
                l = Utilitarios.parseDouble_Null(rs.getString("t_venta"));
    ...
    public List<Object[]> buscar_modeloVehiculoGeneral(Object idtipo, Object idmarca, Object idmodelo, Object idversion, Object anio, Object filtromodelo, Object idcampania, Object idformapago, Object idmoneda, Object fecha) throws Exception {//no se usa
        ...
        try {
            ...
            while (rs.next()) {
                ...
                s[8] = Utilitarios.parseDouble_Null(rs.getString("nimporteventapublico"));
                s[9] = Utilitarios.parseDouble_Null(rs.getString("NDESCUENTO_IMPORTADOR"));
                s[10] = Utilitarios.parseDouble_Null(rs.getString("NDESCUENTO_MAX_AUTORIZADO"));
                s[11] = (rs.getString("aniofab"));
                s[12] = Utilitarios.parseInteger_Null(rs.getString("FK_IDTG20VERSIONVEH"));
...
// usuario,habilitado,orden,url,comando,actualiza,style,styleClass,sub_menu
            ...
            cl.setObject(8, contenido[7]);
            cl.setObject(9, contenido[8]);
            cl.setObject(10, contenido[9]);
            cl.setObject(11, contenido[10].length() > 0 ? Integer.valueOf(contenido[10]) : null);
            cl.setObject(12, contenido[11].length() > 0 ? Integer.valueOf(contenido[11]) : null);
            cl.setObject(13, contenido[12]);
            cl.setObject(14, contenido[13]);
            cl.setObject(15, contenido[14]);
            cl.setObject(16, contenido[15]);
            cl.setObject(17, contenido[16]);
            cl.setObject(18, contenido[17]);
            cl.setObject(19, contenido[18]);
